...
/*
 * Description: Script that activates and deactivates animated, lit, and particle effect props in the RNLowPolyDungeon set. It is included for demonstration purposes,
...
namespace RNLowPolyDungeon
{
    ...
    public class ActivateProp : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            ltFlicker = GetComponentInChildren<LightFlicker>();
            ...
            if (!anim && particleSystems.Length == 0 && !ltSource && !ltFlicker)
        ...
        void OnMouseEnter()
        {
            ...
            Cursor.SetCursor(UIInstructionOverlay.Instance.cursorTexture, UIInstructionOverlay.Instance.cursorTarget, CursorMode.Auto);
        }
        ...
        void Activate(bool setStartingActivation = false)
        {
            ...
            // If the prop's light uses a LightFlicker script, this enables or disables the component.
            if (ltFlicker)
                ltFlicker.enabled = propIsActive;
        }
